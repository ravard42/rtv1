...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/26 21:05:18 by ravard            #+#    #+#             */
/*   Updated: 2017/02/26 21:14:11 by ravard           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		solve(float *sol, float *param)
{
	...
	if ((delt = pow(param[1], 2) - 4 * param[0] * param[2]) == 0)
	{
		...
		sol[0] = (tmp[0] >= VP_DIST) ? tmp[0] : sol[0];
	}
	else if (delt > 0)
	{
...
static void		sol_test(int i, float *sol, t_env *e)
{
	...
	{
		tmp[0] = e->o->co->t->cam_pos[2]
			+ sol[0] * e->o->co->t->cam_r_dir[i][2] - e->o->co->t->obj_pos[2];
		k = 0;
		tmp[1] = e->o->co->t->cam_pos[2]
			+ sol[1] * e->o->co->t->cam_r_dir[i][2] - e->o->co->t->obj_pos[2];
		if (!e->o->co->borne
...
int				cone_test(t_env *e)
{
	...
	while (++i < MAX_X * MAX_Y)
	{
		matrix_product(e->o->co->t->cam_r_dir[i],
				e->o->co->t->mat, e->c->r_dir[i]);
		set_param(i, param, e);
		solve(sol, param);
		sol_test(i, sol, e);
	}

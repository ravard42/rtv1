...
void	print_sphere(t_env *e)
{
	...
	while (++i < MAX_X * MAX_Y)
	{
		...
		param[1] = 2 * (e->c->r_dir[i][0] * (e->c->pos[0] - e->o->s->origin[0]) + e->c->r_dir[i][1] * (e->c->pos[1] - e->o->s->origin[1]) + e->c->r_dir[i][2] * (e->c->pos[2] - e->o->s->origin[2]));
		param[2] = pow(e->c->pos[0] - e->o->s->origin[0], 2) + pow(e->c->pos[1] - e->o->s->origin[1], 2) + pow(e->c->pos[2] - e->o->s->origin[2], 2) - pow(e->o->s->r, 2);
